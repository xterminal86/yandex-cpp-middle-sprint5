...
//
// Добавьте к методам класса Point2D и Lines2DDyn все необходимые аттрибуты и
// спецификаторы
...
struct Point2D
{
  ...
  {
    static Point2D v(std::numeric_limits<int>::max(),
...
struct BoundingBox
{
  ...
  constexpr BoundingBox(double min_x,
                        double min_y,
                        double max_x,
  ...
  [[nodiscard]]
  constexpr bool Overlaps(const BoundingBox &other) const noexcept
  {
...
struct Line
{
  ...
  [[nodiscard]]
  constexpr double Length() noexcept { return start.DistanceTo(end); }

  [[nodiscard]]
  constexpr Point2D Direction() noexcept { return (end - start).Normalize(); }

  [[nodiscard]]
  constexpr BoundingBox BoundBox() const noexcept
  {
      return {
  ...
  [[nodiscard]]
  constexpr double Height() const noexcept { return std::max(start.y, end.y); }

  [[nodiscard]]
  constexpr Point2D Center() noexcept { return (start + end) / 2.0; }

  [[nodiscard]]
  constexpr std::array<Point2D, 2> Vertices() const noexcept
  {
      return {Point2D{start.x, start.y}, {end.x, end.y}};
  }
  [[nodiscard]]
  constexpr Lines2D<2> Lines() const noexcept
  {
    return {
             { start.x, end.x },
             { start.y, end.y }
...
struct Triangle
{
  ...
  constexpr Triangle(Point2D a, Point2D b, Point2D c) noexcept
    : a(a), b(b), c(c) {}
  ...
  [[nodiscard]]
  constexpr std::array<Point2D, 3> Vertices() const noexcept
  {
    return {a, b, c};
  }

  [[nodiscard]]
  constexpr double Height() const noexcept { return std::max({a.y, b.y, c.y}); }

  [[nodiscard]]
  constexpr Point2D Center() const noexcept { return (a + b + c) / 3.0; }

  [[nodiscard]]
  constexpr Lines2D<4> Lines() const noexcept
  {
    return {
             { a.x, b.x, c.x, a.x },
             { a.y, b.y, c.y, a.y }
...
struct Rectangle
{
  ...
  [[nodiscard]]
  constexpr Point2D TopRight() const noexcept
  {
  ...
  [[nodiscard]]
  constexpr std::array<Point2D, 4> Vertices() const noexcept
  {
    return {
             bottom_left,
  ...
  [[nodiscard]]
  constexpr double Height() const noexcept { return bottom_left.y + height; }

  [[nodiscard]]
  constexpr Point2D Center() noexcept
  {
    return bottom_left + (Point2D{width, height} / 2.0);
  }

  [[nodiscard]]
  constexpr Lines2D<5> Lines() const noexcept
  {
    return {
             {
               bottom_left.x,
...
struct RegularPolygon
{
  ...
  [[nodiscard]]
  constexpr double Height() const noexcept { return center_p.y + radius; }

  [[nodiscard]]
  constexpr Point2D Center() const noexcept { return center_p; }

  [[nodiscard]]
  constexpr Lines2DDyn Lines() const
  {
    auto verts = Vertices();
    Lines2DDyn lines;
...
struct Circle
{
  ...
  [[nodiscard]]
  constexpr std::vector<Point2D> Vertices(size_t N = 30) const
  {
    std::vector<Point2D> points;
    points.reserve(N);
  ...
  [[nodiscard]]
  constexpr Lines2DDyn Lines(size_t N = 100) const
  {
    Lines2DDyn lines;
    lines.Reserve(N + 1);
    for (auto i : std::ranges::views::iota(0u, N))
    {
      ...
      lines.PushBack(center_p.x + radius * std::cos(angle),
                     center_p.y + radius * std::sin(angle));
    }
    lines.PushBack(lines.Front());
    return lines;
...
{
  public:
    constexpr Polygon(std::vector<Point2D> points) noexcept :
      points_(std::move(points)) { CalculateBoundBox(); }

    [[nodiscard]]
    constexpr BoundingBox BoundBox() const noexcept { return bounding_box_; }
    ...
    [[nodiscard]]
    constexpr std::span<const Point2D> Vertices() const noexcept
    {
      return points_;
    }
...
struct formatter<vector<geometry::Point2D>>
{
  ...
  template <typename FormatContext>
  auto format(const vector<geometry::Point2D>& v, FormatContext& ctx)
  {
    stringstream ss;
    for (auto& p : v)
    {
      if (use_new_line)
      {
        ss << format_to(ctx.out(), "\t{}\n", p);
      }
      else
      {
        ss << format_to(ctx.out(), "{} ", p);
      }
    }
...
struct formatter<geometry::Shape>
{
  ...
  template <typename FormatContext>
  auto format(const geometry::Shape& shape, FormatContext& ctx)
  {
    shape.visit(
      [&ctx](const auto& s)
      {
